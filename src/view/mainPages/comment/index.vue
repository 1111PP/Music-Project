...
import { getComment, addComment, deleteComment, updateUpvoteComment, getUpvoteComment, setUserStatus } from './services.js'
...
const { currSongTextInfo, songCoverImg, currSongId } = storeToRefs(audioStore)

const bannedUser = async (id) => {
    const r = await setUserStatus({
    ...
    if (r.code === 200) {
        type = 'success'
        message = 'ID为' + id + '的用户已被禁言'
    } else if (r.code === 201) {
        type = 'warning'
        message = '无权禁言'
    }
    ElMessage({ type, message, offset: 339 })
...
const sendComment = async () => {
    ...
    // 🟥添加评论
        ...
        const r = await addComment({ userId: userInfo.value.userId, comment: commentText.value, time: formattedDate, })
        if (r?.code == 200) {
            ElMessage({ offset: 339, message: '评论成功!', type: 'success' })
            // 重新获取评论
            await getCommentAPI()
            commentText.value = ''
        } else {
...
const getCommentAPI = async () => {
    ...
    if (r?.code == 200) {
        ...
        // 2.进一步获取评论的点赞信息
        const r1 = await getUpvoteComment({ songId: currSongId.value, userId: userInfo.value.userId })
        if (r1?.code == 200) {
            // console.log(r1)
            //处理评论与点赞信息
...
onMounted(async () => {
    ...
    const observe = new ResizeObserver((entries) => {
        commentBoxHeight.value = entries[0].contentRect.height - 25
    })
...
onBeforeUnmount(async () => {
    //避免与👇logout时的保存操作冲突
    if (localStorage.getItem('username') !== '') await saveUpvoteComment()
})
...
<template>
    <!-- <VEmojiPicker /> -->
    <div class="comment-container" ref='commentBox'>
        <lpInfiniteScroll v-if="commentList.length > 0" style="background-color: transparent;" :listData="commentList"
            :outHeight="commentBoxHeight" :singlePushCount="5">
            <template #title>
                <div class="top">
                    <div class="cover">
                        <img :src="songCoverImg" alt="">
                    </div>
                    <div class="songInfo">
                        <div class="title">{{ currSongTextInfo.title }}</div>
                        <div class="otherInfo">
                            <span>专辑:{{ currSongTextInfo.ablum }}</span>&nbsp;&nbsp;
                            <span>歌手:{{ currSongTextInfo.author }}</span>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="otherInfo">
                                ...
                                <div class="action">
                                    ...
                                    <div class="like" :data-likeCount="item.likecount">
                                        ...
                                        -->
                                        <svgUpvote :class="item.liked ? 'liked' : ''"
                                            @click="upvoteComment($event, item)" class="icon"
                                            :color="item.liked ? 'red' : 'gray'" />

                                        <!-- 点赞统计数字 = 0不显示,是点赞评论变成红色 -->
                                        <div v-show="item.likecount !== 0"
                                            :style="{ color: item.liked ? 'red' : 'gray', }" class="likecount">
                                            {{
                                                item.likecount
                                            }}</div>
                                    </div>
